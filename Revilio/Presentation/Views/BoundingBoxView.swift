...
class BoundingBoxView {
    ...
    func show(frame: CGRect, label: String, color: UIColor, alpha: CGFloat) {
        ...
        let path = UIBezierPath(roundedRect: frame, cornerRadius: 5.0)  // Rounded rectangle for the bounding box
        ...
        let textRect = label.boundingRect(with: CGSize(width: 500, height: 120),
                options: .truncatesLastVisibleLine,
                attributes: attributes, context: nil)
        let textSize = CGSize(width: textRect.width + 10, height: textRect.height)  // Add padding to the text size
